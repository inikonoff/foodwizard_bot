...
📚 КАТЕГОРИИ:
- "mix" (ПОЛНЫЙ ОБЕД) — ОБЯЗАТЕЛЬНО ПЕРВЫМ, если продуктов >= 12.
- "soup", "main", "salad", "breakfast", "dessert", "drink", "snack".

🎯 ТРЕБОВАНИЯ:
1. Если продуктов >= 12, верни "mix" и еще 3 подходящие категории.
2. Если продуктов < 12, верни от 2 до 4 категорий.
...
🎯 ТРЕБОВАНИЯ К ЯЗЫКУ:
- Поле "name": СТРОГО на языке ввода ({safe_products}). НЕ ПЕРЕВОДИ.
- Поле "desc": СТРОГО на РУССКОМ языке.

🎯 ТРЕБОВАНИЯ К МЕНЮ:
- СТРОГО 4 блюда: Суп, Второе блюдо, Салат, Напиток.
- Главный белок: 30% в суп, 70% во второе.
🎯 JSON: [{{ "name": "Название на языке ввода", "desc": "Описание на русском" }}]"""
        else:
            prompt = f"""📝 ЗАДАНИЕ: Составь меню "{category}".
🛒 ПРОДУКТЫ: {safe_products}
{base_instruction}
🎯 ТРЕБОВАНИЯ К ЯЗЫКУ:
- Поле "name": СТРОГО на языке ввода. НЕ ПЕРЕВОДИ.
- Поле "desc": СТРОГО на РУССКОМ языке.
🎯 JSON: [{{"name": "...", "desc": "..."}}]"""
...
🎯 ТРЕБОВАНИЯ К ЯЗЫКУ:
1. Заголовок и Названия ингредиентов: Оставь СТРОГО на языке оригинала (как в "{safe_products}").
2. В скобках рядом с каждым ингредиентом напиши перевод на РУССКИЙ (например: "Pollo (курица) — 500 г").
3. Шаги приготовления и Совет: Пиши СТРОГО на РУССКОМ языке.
...
{safe_dish_name}
📦 Ингредиенты:
- [Ингредиент на языке оригинала] ([перевод на русский]) — [количество]
...
🎯 Название и ингредиенты — на языке оригинала, перевод на русский в скобках. Шаги — на русском.
